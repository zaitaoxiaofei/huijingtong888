...
| Future database evolution | In progress | Keep SQLite stable as the default runtime while preparing a phased migration path to MySQL. | Execution source of truth: `docs/MYSQL_PREPARATION_PLAN.md`. Start with config standardization, db adapter extraction, and schema/migration/bootstrap separation. |
| Database configuration standardization | Done | Introduce a dual-database-ready config contract while keeping SQLite as the active default runtime. | Added `DB_CLIENT`, `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`, `DB_POOL_MIN`, `DB_POOL_MAX`, and startup validation in `src/config.js`; updated `.env.example` and deployment env example. |
| Database adapter extraction | In progress | Remove direct dependency on SQLite client APIs from service and session layers. | Added initial SQLite-backed adapter module with `queryOne`, `queryAll`, `execute`, `insertAndGetId`, and `runInTransaction`; `src/server/session.js`, `src/services/configuration.js`, key write paths in `src/services/orders.js`, inventory rule / stock snapshot writes in `src/services/inventory.js`, analytics snapshot refresh execution in `src/services/analytics-refresh.js`, finance sync / finance reapply write paths in `src/services/finance-sync.js`, core order sync write / insert paths in `src/services/order-sync.js`, online product entry/runtime writes in `src/services/online-products-entry.js`, catalog mapping maintenance writes in `src/services/catalog-mappings-entry.js`, most configuration CRUD writes in `src/services/configuration.js`, procurement / purchase-order / inbound transaction flows in `src/services.js`, `recalculateOrderCancelLossFlags()` in `src/services.js`, and historical profit review transaction flow now support adapter-style execution or adapter-owned transaction control instead of hard-binding to raw `db.prepare()` / `db.exec()` calls. |
| Adapter regression tests | In progress | Every high-value migrated path should gain executable regression coverage before more cutover work relies on it. | Added `npm test` based on Node `node:test` and created `test/service-adapter.test.js` covering adapter-backed execution for analytics refresh, finance sync, and order sync. Expand this test set as more modules migrate. |
| Schema and bootstrap separation | In progress | Split runtime client creation from schema creation, migration steps, seed logic, and repair logic. | Runtime SQLite connection creation has been extracted into `src/database-client.js`, seed/bootstrap logic has been extracted into `src/db-bootstrap.js`, safe repair/cleanup execution now routes through `src/db-repairs.js`, and schema/index extraction has started in `src/db-schema.js`; `applyBaseSchema()` and index/supplemental helpers are now part of the active init/migration execution flow, while `src/db.js` still retains the legacy base-schema SQL block, migration concerns, and the local cancellation-rule corruption repair for compatibility. Current blocker is safely disabling or removing the duplicated top-level base-schema execution block inside the encoding-damaged `src/db.js` file. |
| SQL dialect compatibility audit | In progress | Maintain a list of SQLite-specific SQL patterns that require replacement or abstraction before MySQL support is enabled. | Initial checklist has been created in `docs/SQLITE_TO_MYSQL_COMPATIBILITY_CHECKLIST.md`, covering `PRAGMA`, `sqlite_master`, `INSERT OR IGNORE`, `ON CONFLICT`, autoincrement handling, booleans, timestamps, transactions, and priority translation targets. |
| MySQL schema draft and export tooling | In progress | Produce a concrete MySQL target schema and a repeatable SQLite export input for dry-run migration. | Added `docs/MYSQL_SCHEMA_DRAFT.md`, `npm run db:export:mysql`, `npm run db:verify:mysql-export`, `npm run db:init:mysql`, `npm run db:import:mysql`, and `npm run db:compare:mysql`; current local SQLite export contains 38 tables and 72,089 rows, and the remaining blocker for full dry-run import is a reachable MySQL instance plus real connection env values. |
| Backup/restore strategy evolution | Backlog | Redesign backup and restore for a future MySQL runtime without breaking the current SQLite safety workflow. | Current scripts and docs are SQLite file/WAL based and must stay authoritative until MySQL backup validation exists. |
...
- 5. When SQLite becomes a write/read bottleneck, migrate this layered model to MySQL through the phased plan in `docs/MYSQL_PREPARATION_PLAN.md` rather than lifting the current mixed model directly.

## 10.2 MySQL Preparation Milestones

| Milestone | Status | Exit condition | Notes |
| --- | --- | --- | --- |
| M0: Preparation plan approved | Done | Team agrees MySQL is a phased program, not a direct switch. | Source doc: `docs/MYSQL_PREPARATION_PLAN.md`. |
| M1: Dual-db config contract | Done | Runtime config supports both SQLite and MySQL fields without changing default behavior. | Implemented in `src/config.js`; example env values documented in `.env.example` and `docs/remote-access-deployment.md`. |
| M2: DB adapter introduced | In progress | New code can use adapter methods instead of `db.prepare()` directly. | Initial adapter is in place and adopted by session/configuration plus selected orders/inventory write paths; SQLite remains the first backing implementation and a few loop-oriented prepared statements are intentionally still local. |
| M3: Schema/migration/bootstrap split | In progress | `src/db.js` no longer owns all database responsibilities. | Current progress: runtime SQLite connection creation moved to `src/database-client.js`, seed/bootstrap logic moved to `src/db-bootstrap.js`, safe repair/cleanup execution now routes through `src/db-repairs.js`, and new schema helper module `src/db-schema.js` now participates in the live init flow through `applyBaseSchema()`, `applySupplementalSchema()`, and `applySqliteIndexes()`; remaining work is deleting the duplicate base-schema SQL from `src/db.js`, extracting ordered migrations, and isolating the encoded cancellation-rule data patch. |
| M4: SQL compatibility checklist complete | In progress | All known SQLite-only patterns are inventoried and mapped to a replacement approach. | Initial checklist document now exists in `docs/SQLITE_TO_MYSQL_COMPATIBILITY_CHECKLIST.md`; remaining work is expanding file-level coverage and linking each risky SQL path to an implementation plan. |
| M5: MySQL schema draft | Backlog | Each current runtime table has a proposed MySQL shape and index strategy. | Do after M1-M4 materially stabilize. |
| M6: Local dry-run migration | Backlog | A real SQLite snapshot can be migrated and verified locally in MySQL. | Includes row-count and key business-path verification. |

...
- `In progress`: sync-domain implementation is being extracted into real modules such as `src/services/finance-sync.js` and `src/services/order-sync.js`; the unused global-runtime forwarding entry was removed.
...
- `In progress`: catalog product read/write and procurement boundaries still require extraction of their real implementations; unused global-runtime forwarding entries were removed.
- `In progress`: inventory-domain implementation boundaries remain under review; the unused global-runtime forwarding entry was removed because no production code imported it.
...
- `In progress`: order-operation implementation boundaries remain under review; the unused global-runtime forwarding entry was removed because no production code imported it.
- `In progress`: analytics snapshot calculation lives in `src/services/analytics-refresh.js`; the unused legacy runtime entry was removed because its synchronous SQLite contract was not compatible with the active asynchronous MySQL implementation.
...
| Backend modularization | In progress | Continue lowering coupling in `server.js`, `services.js`, and the runtime frontend entry through phased module boundaries. | Route-layer extraction covers order/profit/sync plus catalog/config/procurement CRUD groups under `src/server/routes/*`. Shared dashboard/finance summary, finance sync, order sync, analytics calculation, and order profit recalculation logic now have domain modules. Catalog and procurement use their directly imported entry modules. Unused forwarding barrels and global-runtime entries were removed instead of preserving artificial boundaries. Next step is moving more real MySQL domain implementations out of the giant files. |
| MySQL domain extraction | In progress | Move only cohesive implementations that reduce coupling while preserving the existing export contract. | Procurement plan/list normalization, split-shipping and package-label utilities, order-profit detail snapshot persistence, online-product status/SKU handling, inventory-alert SKU snapshot refresh, and FBP read-model normalization now live in dedicated MySQL modules. `mysql-cutover.js` keeps compatibility exports and thin runtime wrappers. |
...
## 8.1 Listing Automation

| Item | Status | Requirement | Notes |
| --- | --- | --- | --- |
| Category publish diagnostics | Done | Publish preflight should show whether the selected Ozon category pair, cached attributes, cached required attributes, and dictionary values are ready for publish. | `validate-publish` now returns `category_health`; cached required attributes can block publish when missing from the final payload, while missing cache/value freshness issues appear as diagnostics. The publish validation drawer displays the checks and dictionary counts. |
| Import-info error UX | Done | Ozon import results should show item-level failure reason and actionable fix guidance. | `/v1/product/import/info` errors are normalized into `import_errors` with offer, field, code, raw Ozon reason, and fix tip. Publish result and publish-record views display the summary and details. |
| Multi-shop publish result handling | Done | Multi-shop Ozon submit results should help operators focus on failed shops and continue into the retry workflow. | Submit result drawer shows success/failure counts, failed-only filtering, and a direct jump to the generated publish record. The publish-record page can auto-open a record from `recordId` route query. |
| Ozon repair navigator | Done | When one publish record has multiple Ozon errors, operators should see every issue and jump to the relevant editable field. | Publish-record detail now lists all normalized import errors, maps common field errors to existing editable controls, and highlights every affected field while keeping unsupported category/attribute errors visible as repair items. |
| Collector-box source quality gate | Done | Collector-box rows that only contain frontend fallback data must not create fake listing templates. | Rows missing title, image, or real Ozon category are flagged as incomplete in the collector box. Backend template creation also blocks `frontend:{sku}` placeholder category data unless explicitly overridden for debugging. |
| Collector plugin fast add | Done | Manual "加入采集箱" should not block on slow Ozon full-detail collection. | The plugin now saves a fast payload from the current panel first, then runs full-detail collection as background backfill. Backfill is deduped per SKU and limited to 12 seconds so slow Ozon entrypoint, rich description, or variant requests do not stack up. |
| Collector-box lightweight list | Done | Opening the collector box should not parse large payload JSON for every row. | Collector rows now store `quality_status`, `quality_issues_json`, and `facts_status` at write time. The list query returns these lightweight fields and skips `edit_payload_json`; full payloads remain lazy-loaded by the detail API. |

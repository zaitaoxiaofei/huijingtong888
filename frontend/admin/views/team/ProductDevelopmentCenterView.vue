...
const ideas = ref([]);
const ideaBrands = ref([]);
const ideaCategories = ref([]);
const developmentBrandFilter = ref("");
const developmentCategoryFilter = ref("");
function developmentScopes(row) { return row.development_scopes?.length ? row.development_scopes : [{ brand: row.development_brand, category: row.development_category }]; }
const developmentBrandOptions = computed(() => [...new Set([...ideas.value, ...tasks.value].flatMap(row => developmentScopes(row).map(scope => scope.brand)).filter(Boolean))].sort());
const developmentCategoryOptions = computed(() => [...new Set([...ideas.value, ...tasks.value].flatMap(row => developmentScopes(row).map(scope => scope.category)).filter(Boolean))].sort());
function matchesDevelopmentScope(row) {
  return developmentScopes(row).some(scope => (!developmentBrandFilter.value || scope.brand === developmentBrandFilter.value)
    && (!developmentCategoryFilter.value || scope.category === developmentCategoryFilter.value));
}
const ideaVisible = ref(false);
const ideaScopeVisible = ref(false);
const ideaTasksVisible = ref(false);
const ideaLinkedTasks = ref([]);
...
const ideaForm = reactive({ id: null, title: "", image_url: "", source_url: "", note: "", urgency: 5, importance: 5,
  assignee_person_id: null, target_product_count: 1, development_due_at: "", development_brand: "", development_category: "", development_tasks: [] });
const tasks = ref([]);
const bindings = ref([]);
...
const taskCreationVisible = ref(false);
const heatmapView = ref(null);
const taskCreationInitial = ref(null);
const developmentTaskVisible = ref(false);
const developmentTask = ref(null);
...
const filteredTasks = computed(() => tasks.value.filter((row) => {
  if (!matchesDevelopmentScope(row)) return false;
...
const taskPeopleGroups = computed(() => {
  ...
  for (const row of audienceTasks.value) {
    const personId = Number(row.owner_person_id || row.assignee_person_id || 0);
    const key = personId || "unassigned";
    if (!groups.has(key)) groups.set(key, { id: personId || null, name: row.owner_name || row.assignee_name || "未分配", rows: [] });
...
const timelineTasks = computed(() => {
  ...
  return [...visibleTasks.value].sort((a, b) => taskTimelineX(a) - taskTimelineX(b) || taskProgress(a) - taskProgress(b)).map((row) => {
    const timelineX = taskTimelineX(row); const progress = taskProgress(row); const bucket = Math.round(timelineX / 8); const lane = occupied.get(bucket) || 0; occupied.set(bucket, lane + 1);
    const offset = lane ? Math.ceil(lane / 2) * 14 * (lane % 2 ? 1 : -1) : 0;
    return { ...row, timelineX, timelineY: Math.max(-12, Math.min(96, progress + offset)), timelineLane: lane };
  });
});
...
const taskListSummary = computed(() => ({
  ...
  skuDone: taskListRows.value.reduce((sum, row) => sum + Number(row.done || 0), 0)
}));
const paginatedTasks = computed(() => taskListRows.value.slice((taskPage.value - 1) * taskPageSize.value, taskPage.value * taskPageSize.value));
...
async function loadTaskOperationalDetails(row) { taskOperationalDetails.value = { rows: [], summary: {} }; taskDetailReason.value = "all"; taskDetailPage.value = 1; if (!isDailyOperationalTask(row) || !row.id) return; taskDetailLoading.value = true; try { const details = await apiClient.get(`/api/team/tasks/${row.id}/operational-details`, { noCache: true }); taskOperationalDetails.value = details; if (details.progress) { Object.assign(row, details.progress); const source = tasks.value.find((item) => Number(item.id) === Number(row.id)); if (source) Object.assign(source, details.progress); } } catch (error) { ElMessage.error(error.message || "任务明细加载失败"); } finally { taskDetailLoading.value = false; } }
...
function openDevelopmentTask(row) {
  if (row.type !== "product_development") return openTask(row);
  if (row.status === "todo") return createTaskDraft(row, developmentModels(row)[0] || {});
  developmentTask.value = row;
  developmentTaskVisible.value = true;
}
function developmentModels(row = developmentTask.value) { return row?.development_plan?.models || []; }
function createTaskDraft(row, model) {
  developmentTaskVisible.value = false;
  router.push({ path: "/listing-automation", query: { workbenchId: `development-${row.id}-${Date.now()}`, quickCreate: "1", from: "development-task", taskId: String(row.id), modelId: String(model.model_id || ""), model: model.model || "" } });
}
function bindTaskDraft(row) {
  developmentTaskVisible.value = false;
...
async function openIdea(row = null) {
  if (row?.task_id) return openTask(row.task);
  try {
    const [brands, categories] = await Promise.all([loadInventoryVehicleCatalog(), loadInventoryNamingOptions({ type: "category" })]);
    ideaBrands.value = brands; ideaCategories.value = categories;
  } catch (error) { return ElMessage.error(error.message || "开发品牌和类目加载失败，请重试"); }
  Object.assign(ideaForm, row ? { id: row.id, title: row.title, image_url: row.image_url || "", source_url: row.source_url || "", note: row.note || "", urgency: row.urgency || 5, importance: row.importance || 5,
    development_brand: row.development_brand || "", development_category: row.development_category || "",
    development_tasks: JSON.parse(JSON.stringify(row.development_tasks || [])),
    ...
    : { id: null, title: "", image_url: "", source_url: "", note: "", urgency: 5, importance: 5, assignee_person_id: null, target_product_count: 1, development_due_at: "", development_brand: "", development_category: "", development_tasks: [] });
...
async function saveIdea() {
  if (!ideaForm.development_tasks.length) return ElMessage.warning("请打开开发坐标表，选择类目、品牌和车型");
...
async function viewIdeaTask(row) {
  ...
  const children = tasks.value.filter(item => item.source_idea_id === row.id);
  if (children.length === 1) return openTask(children[0]);
  if (children.length > 1) { ideaLinkedTasks.value = children; ideaTasksVisible.value = true; return; }
  const task = tasks.value.find(item => item.idea_id === row.id);
  if (task) openTask(task);
  else ElMessage.warning("关联任务暂未加载，请刷新后重试");
}
function canClaimIdea(row) {
  return !row.task_id && ["idea", "assigned"].includes(row.status) && !row.claimed_at
    && (!row.assignee_person_id || Number(row.assignee_person_id) === Number(authStore.user?.personId || authStore.user?.id || 0));
}
async function claimIdea(row) {
  if (claimingIdeaId.value !== null) return;
  claimingIdeaId.value = row.id;
  ...
  finally { claimingIdeaId.value = null; }
}
...
function openTask(row = null) {
  if (row?.idea_id) {
    loadDevelopmentData().then(() => {
      const idea = ideas.value.find(item => item.id === row.idea_id);
      if (idea) openIdea(idea);
      else ElMessage.warning("关联灵感已不可用，请刷新列表");
    });
    return;
  }
  if (!row || row.development_plan) {
    taskCreationInitial.value = row;
    taskCreationVisible.value = true;
    return;
  }
...
async function saveTask() {
  if (isDailyOperationalTask(taskForm) && !taskForm.owner_person_id) return ElMessage.warning("请选择当前任务负责人后保存");
...
<template>
  <div v-loading="loading" class="development-center">
    ...
    <div v-if="['ideas', 'tasks'].includes(activeTab)" class="development-scope-filters">
      <el-select v-model="developmentBrandFilter" clearable filterable placeholder="全部品牌 / 非汽车" aria-label="筛选开发品牌"><el-option v-for="brand in developmentBrandOptions" :key="brand" :label="brand" :value="brand" /></el-select>
    ...
    <section v-else-if="activeTab==='ideas'" class="panel ideas-page">
      ...
      <div class="idea-grid"><article v-for="row in sortedIdeas" :key="row.id" class="idea-card">
        <div class="idea-card-media"><el-image :src="row.image_url" fit="cover" :preview-src-list="row.image_url?[row.image_url]:[]" preview-teleported><template #error><div class="empty-image">暂无主图</div></template></el-image><el-button v-if="!row.task_id" class="replace-image-button" size="small" :icon="ImagePlus" @click="openIdea(row)">替换图片</el-button></div>
        <div class="idea-card-content"><header><div class="idea-title-line"><h3>{{ row.title }}</h3><span class="task-count">任务 {{ row.target_product_count || 0 }}</span><span class="priority-badge urgent">紧急 {{ row.urgency }}</span><span class="priority-badge important">重要 {{ row.importance }}</span></div><p>{{ row.note || '暂无备注' }}</p><el-link v-if="row.source_url" :href="row.source_url" target="_blank" type="primary" :underline="false">查看参考链接</el-link></header>
          <div class="idea-meta-row"><span><b>开发类目</b>{{ row.development_category || '待补充' }}</span><span><b>汽车品牌</b>{{ row.development_brand || '待补充' }}</span></div><div class="idea-people-row"><span><b>指定人员</b><el-avatar :src="personAvatar(row.assignee_person_id)" :size="24">{{ personInitial(row.assignee_name) }}</el-avatar>{{ row.assignee_name || '待指定' }}</span><span><b>创建人</b><el-avatar :src="personAvatar(row.created_by_person_id)" :size="24">{{ personInitial(row.created_by_name || '系统') }}</el-avatar>{{ row.created_by_name || '系统' }}</span></div>
          ...
          <footer><el-button v-if="canClaimIdea(row)" type="primary" :loading="claimingIdeaId === row.id" :disabled="claimingIdeaId !== null && claimingIdeaId !== row.id" @click="claimIdea(row)">认领任务</el-button><el-tag v-else-if="row.claimed_at" type="success">已认领</el-tag><el-button :icon="Edit3" plain @click="openIdea(row)">编辑</el-button><el-button v-if="!row.task_id && !row.development_tasks?.length && row.status==='idea'" plain @click="startIdeaDevelopment(row)">去开发</el-button><el-button type="primary" plain @click="viewIdeaTask(row)">查看任务</el-button></footer>
        </div>
    ...
    <section v-else-if="activeTab==='tasks'" class="tasks-page">
      ...
      <el-dialog v-model="taskListVisible" :title="`${taskPeopleTitle}清单`" width="min(1440px, 94vw)" top="4vh" class="task-list-dialog" destroy-on-close>
        <el-table :data="paginatedTasks" max-height="calc(100vh - 280px)" empty-text="当前筛选下没有任务">
          ...
          <el-table-column label="当前进度" width="200"><template #default="{row}"><div class="task-progress-cell"><el-progress :percentage="taskProgress(row)" :show-text="false" :stroke-width="7" :status="taskTone(row)==='overdue'?'exception':taskTone(row)==='done'?'success':undefined" /><span>{{ taskProgress(row) }}%</span></div></template></el-table-column>
          ...
          <el-table-column label="操作" width="132" fixed="right" align="center"><template #default="{row}"><div class="task-row-actions"><el-button v-if="row.type==='product_development'" size="small" type="primary" plain @click="openDevelopmentTask(row)">{{ row.status==='todo' ? '去开发' : 'SKU' }}</el-button><el-tooltip v-else content="查看任务" placement="top"><el-button circle :icon="Edit3" @click="openTask(row)" /></el-tooltip><el-tooltip v-if="!row.automation_key" content="删除任务" placement="top"><el-button circle :icon="Trash2" type="danger" plain @click="deleteTask(row)" /></el-tooltip></div></template></el-table-column>
        </el-table>
    ...
    <el-dialog v-model="ideaVisible" class="idea-edit-dialog" :title="ideaForm.id ? '编辑灵感' : '快速创建灵感'" width="min(720px, 94vw)" align-center @paste="handleIdeaImagePaste"><el-form label-position="top"><el-form-item label="标题" required><el-input v-model="ideaForm.title" placeholder="一句话记录这个产品灵感" /></el-form-item><el-form-item label="开发范围与任务分组" required><div style="width:100%"><el-button type="primary" plain @click="ideaScopeVisible=true">打开开发坐标表 · 多选类目和车型</el-button><p v-if="!ideaForm.development_tasks.length">{{ [ideaForm.development_brand, ideaForm.development_category].filter(Boolean).join(' · ') || '尚未选择开发范围' }} · 请在坐标表中选择车型</p><div v-for="(group,index) in ideaForm.development_tasks" :key="group.key" style="padding:10px 0;border-bottom:1px solid #edf1f7"><el-tag>任务 {{ index+1 }}</el-tag> {{ [...new Set(group.models.map(row => `${row.brand} · ${row.category}`))].join(' / ') }}<small style="display:block;color:#64748b;margin-top:6px">{{ group.models.map(row => `${row.model} × ${row.target}`).join('；') }}</small></div><p>共 {{ ideaForm.development_tasks.length }} 个任务 · {{ ideaForm.development_tasks.flatMap(group => group.models).reduce((sum,row) => sum+Number(row.target||0),0) }} 个 SKU</p></div></el-form-item><el-form-item label="主图"><div class="idea-image-editor"><button type="button" class="idea-image-dropzone" :class="{ uploading: ideaImageUploading }" @click="chooseIdeaImage" @dragover.prevent @drop.prevent="handleIdeaImageDrop"><el-image v-if="ideaForm.image_url" :src="ideaForm.image_url" fit="cover"><template #error><div class="empty-image">图片无法预览</div></template></el-image><div v-else class="idea-image-placeholder"><ImagePlus :size="30" /><strong>{{ ideaImageUploading ? '正在上传…' : '点击、拖拽或粘贴图片' }}</strong><span>支持 Ctrl+V 粘贴，建议使用 3:4 图片</span></div><div v-if="ideaForm.image_url" class="idea-image-change"><Upload :size="15" /> 更换图片</div></button><div class="idea-image-link"><span>也可以添加图片链接</span><el-input v-model="ideaForm.image_url" clearable placeholder="https://..." /></div><input ref="ideaFileInput" class="hidden-file-input" type="file" accept="image/*" @change="handleIdeaFileChange" /></div></el-form-item><el-form-item label="参考链接"><el-input v-model="ideaForm.source_url" placeholder="1688、Ozon、WB 或其他链接" /></el-form-item><el-form-item label="备注"><el-input v-model="ideaForm.note" type="textarea" :rows="3" placeholder="补充卖点、成本判断或需要验证的问题" /></el-form-item><div class="form-grid"><el-form-item label="紧急度（1-10）"><el-slider v-model="ideaForm.urgency" :min="1" :max="10" show-input /></el-form-item><el-form-item label="重要度（1-10）"><el-slider v-model="ideaForm.importance" :min="1" :max="10" show-input /></el-form-item><el-form-item label="指定人员"><el-select v-model="ideaForm.assignee_person_id" filterable clearable placeholder="选择负责人"><el-option v-for="person in people" :key="person.id" :label="person.name" :value="person.id" /></el-select></el-form-item><el-form-item label="截止时间"><el-date-picker v-model="ideaForm.development_due_at" type="date" value-format="YYYY-MM-DD" placeholder="选择截止日期" /></el-form-item></div></el-form><template #footer><el-button @click="ideaVisible=false">取消</el-button><el-button type="primary" :loading="ideaSaving" :disabled="ideaImageUploading" @click="saveIdea">保存灵感</el-button></template></el-dialog>
...
<style>
.idea-edit-dialog .el-dialog__body { max-height: 72vh; overflow-y: auto; }
</style>
